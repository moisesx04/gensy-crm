      ...
      <div style={{
        ...
        <div className="form-main-card" style={{
          ...
          <div style={{
            ...
            background: 'linear-gradient(160deg, #0d1e5c 0%, #1a0a4a 100%)',
            padding: '48px 36px',
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 36 }}>
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 20, boxShadow: '0 4px 16px rgba(79,110,247,.4)', flexShrink: 0,
              }}>🏡</div>
              <div>
                <div style={{ color: '#fff', fontWeight: 800, fontSize: 16 }}>GENSY</div>
                <div style={{ color: 'rgba(255,255,255,.4)', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '1px' }}>Inmobiliario</div>
              </div>
            </div>

            {/* Steps sidebar */}
            <div style={{ flex: 1 }}>
              <p style={{ color: 'rgba(255,255,255,.4)', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '1.5px', fontWeight: 700, marginBottom: 16 }}>
                Tu progreso
              </p>
              {STEPS.map((s, i) => {
                const SIcon = s.icon;
                ...
                const active = i === step;
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6, position: 'relative' }}>
                    {/* Connector line */}
                    {i < STEPS.length - 1 && (
                      <div style={{
                        position: 'absolute', left: 14, top: 32, width: 1.5, height: 18,
                        background: done ? '#4f6ef7' : 'rgba(255,255,255,.1)',
                        transition: 'background .3s',
                      }} />
                    )}
                    <motion.div
                      animate={{
                        background: done ? '#4f6ef7' : active ? 'linear-gradient(135deg,#4f6ef7,#e84f8c)' : 'rgba(255,255,255,.08)',
                        scale: active ? 1.1 : 1,
                      }}
                      style={{
                    ...
                    >
                      {done
                        ...
                        : <SIcon size={14} color={active ? '#fff' : 'rgba(255,255,255,.35)'} />
                      }
                    </motion.div>
                      <div>
                        <div style={{
                          fontSize: 15, fontWeight: 700,
                          color: active ? '#fff' : done ? 'rgba(255,255,255,.7)' : 'rgba(255,255,255,.35)',
                          transition: 'color .25s',
                        }}>{s.label}</div>
                        {active && (
                          <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,.45)' }}>{s.sub}</div>
                        )}
                      </div>
                  </div>
                );
            ...
            <div style={{ marginTop: 28, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,.08)' }}>
              {[
                ...
                <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <Ic size={13} color="rgba(255,255,255,.3)" />
                  <span style={{ fontSize: 11.5, color: 'rgba(255,255,255,.3)', fontWeight: 600 }}>{text}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="form-right-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            ...
            <div className="form-header-bar" style={{
              ...
              <div style={{
                ...
                background: 'linear-gradient(135deg, #edf0ff, #f3f1ff)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                <Icon size={17} color="#4f6ef7" />
              </div>
              <div>
                <h2 style={{ fontSize: 21, fontWeight: 800, color: '#0d1630', margin: 0 }}>{S.title}</h2>
                <p style={{ fontSize: 14.5, color: '#9aa3be', margin: 0, marginTop: 1 }}>{S.sub}</p>
              </div>
              {/* Progress dots */}
              <div style={{ display: 'flex', gap: 8, marginLeft: 'auto' }}>
                {STEPS.map((_, i) => (
                  <div key={i} style={{
                    ...
                    width: i === step ? 30 : 8,
                    background: i < step ? '#4f6ef7' : i === step ? 'linear-gradient(90deg,#4f6ef7,#e84f8c)' : '#e4e9f4',
            ...
            <form onSubmit={submit} style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  >
                        ...
                        <FInput label="¿Cuántas personas?" required>
                          <div style={{ position: 'relative' }}>
                            ...
                            <input required type="number" min="1" max="20" value={f.numPersonas}
                              ...
                              style={{ ...inp('np'), paddingLeft: 42 }} placeholder="Ej: 4" />
                          </div>
                        </FInput>
                        <FInput label="Habitaciones requeridas" required>
                          <div style={{ position: 'relative' }}>
                            ...
                            <input required type="number" min="0" max="10" value={f.numHabitaciones}
                              ...
                              style={{ ...inp('nh'), paddingLeft: 42 }} placeholder="Ej: 3" />
                          </div>
                        </FInput>
                      </div>
                        ...
                        <FInput label="Teléfono (Opcional)">
                          <div style={{ position: 'relative' }}>
                            ...
                            <input value={f.telefono}
                              ...
                              style={{ ...inp('tel'), paddingLeft: 42 }} placeholder="Celular o residencial" />
                          </div>
                        </FInput>
                        <FInput label="Dirección actual (Opcional)">
                          <div style={{ position: 'relative' }}>
                            ...
                            <input value={f.direccion}
                              ...
                              style={{ ...inp('dir'), paddingLeft: 42 }} placeholder="Calle, apt, ciudad" />
                          </div>
                        </FInput>
                      </div>
                    ...
                    {step === 1 && (<div>
                      <FInput label="Tipo de Identificación" required>
                        <div className="mobile-opt-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(170px,1fr))', gap: 8 }}>
                      ...
                      <FInput label="Credit Score (opcional)" hint="Déjalo en blanco si no lo sabes.">
                        <div style={{ position: 'relative' }}>
                          ...
                          <input type="number" min="300" max="850" value={f.creditScore}
                            ...
                            style={{ ...inp('cs'), paddingLeft: 42 }} placeholder="Ej: 680" />
                        </div>
                      </FInput>
                    </div>)}
                    ...
                    {step === 2 && (<div>
                      ...
                      <FInput label="¿Cómo cobra su salario?" required>
                        <div className="mobile-opt-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                        ...
                        {f.presentoTaxes === 'Sí' && (
                          <motion.div initial={{ opacity:0,height:0 }} animate={{ opacity:1,height:'auto' }}
                            ...
                            <FInput label="Monto de ingresos en los taxes">
                              <div style={{ position:'relative' }}>
                                <span style={{ position:'absolute',left:13,top:'50%',transform:'translateY(-50%)',fontWeight:700,color:'#9aa3be',pointerEvents:'none' }}>$</span>
                                <input type="number" value={f.montoTaxes} onChange={e => set('montoTaxes', e.target.value)}
                                  ...
                                  style={{ ...inp('mt'), paddingLeft: 28 }} placeholder="Ej: 45000" />
                              </div>
                            </FInput>
                          </motion.div>
                        )}
                      ...
                      <FInput label="Ingresos mensuales totales del hogar" required hint="Suma de todos los ingresos de las personas que van a aplicar.">
                        <div style={{ position:'relative' }}>
                          ...
                          <input required type="number" value={f.ingresosMensuales} onChange={e => set('ingresosMensuales', e.target.value)}
                            ...
                            style={{ ...inp('im'), paddingLeft: 28 }} placeholder="Ej: 3500" />
                        </div>
                      </FInput>
                    </div>)}
                        ...
                        {(f.cashOPrograma === 'Programas de asistencia' || f.cashOPrograma === 'Ambos (Cash + Programa)') && (
                          <motion.div initial={{ opacity:0,height:0 }} animate={{ opacity:1,height:'auto' }}
                            ...
                            <FInput label="¿Qué programa de asistencia utiliza?">
                              <div className="mobile-opt-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 8 }}>
                                {PROGRAMAS.map(v => (
                    ...
                    {/* ── STEP 4: Revisión ── */}
                    {step === 4 && (<div>
                      <div style={{ background: '#fafbff', borderRadius: 14, border: '1.5px solid #e4e9f4', overflow: 'hidden', padding: '0 16px' }}>
              ...
              <div className="form-footer-nav" style={{
                ...
                {step > 0 ? (
                  <motion.button type="button" whileTap={{ scale: 0.97 }} onClick={goBack}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 8,
                      padding: '12px 24px', borderRadius: 12, border: '2px solid #e4e9f4',
                ...
                {step < 4 ? (
                  <motion.button type="button" whileTap={{ scale: 0.97 }} onClick={goNext}
                    ...
                    style={{
                      ...
                      background: canAdvance() ? 'linear-gradient(135deg, #4f6ef7, #7c3aed)' : '#e4e9f4',
                      fontFamily: 'inherit', fontSize: 15, fontWeight: 700,
                      color: canAdvance() ? '#fff' : '#b0b8d4',
                      cursor: canAdvance() ? 'pointer' : 'not-allowed',
                      boxShadow: canAdvance() ? '0 4px 16px rgba(79,110,247,.3)' : 'none',
                    }}>
                  ...
                  <motion.button type="submit" whileTap={{ scale: 0.97 }} disabled={submitting}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 10,
                      padding: '13px 36px', borderRadius: 12, border: 'none',
                      background: submitting ? '#e4e9f4' : 'linear-gradient(135deg, #17c98a, #059669)',
                      fontFamily: 'inherit', fontSize: 15, fontWeight: 700, color: '#fff',
                      cursor: submitting ? 'wait' : 'pointer',
                      boxShadow: submitting ? 'none' : '0 4px 16px rgba(23,201,138,.3)',
                    }}>
                    {submitting ? '⏳ Enviando...' : <><Check size={18} /> Enviar Solicitud</>}
                  </motion.button>
                )}
              </div>
            </form>
        ...
        <style>{`
          @media (max-width: 850px) {
            ...
            .form-right-panel { padding: 30px 20px !important; }
            .form-header-bar { padding: 20px !important; }
            .form-footer-nav { padding: 20px !important; flex-wrap: wrap; gap: 15px; }
            .form-footer-nav button { width: 100% !important; justify-content: center; }
            .mobile-grid-1 { grid-template-columns: 1fr !important; }
            .mobile-opt-grid { grid-template-columns: 1fr 1fr !important; }
          }
